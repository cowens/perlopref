...
C<X = X - 1>.  The value returned is reflects the decementing, so after the
...
C<X = X - 1>.  The value returned is the value before decementing, so after
...
negation, i.e., "not".  There is a low-prcedence version C<not>.
...
The unary minus operator performs arithmetic negation if X is numeric.  If
is a bareword it returns a string consisting of - and the bareword.
