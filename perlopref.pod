...
This is the array/list index operator.  It retrieves the Yth value
from X.  If Y is a list then a slice is returned (see L<perldata> for
information about slices).  Indexes are C<0> based (unless C<$[> has 
been set to a non-zero value). Negative indexes count backwards from
the end of the list or array.
...
This is the hash index operator.  It retrieves the value associated with
the key Y from X.  If Y is a list of keys then a slice is returned (see 
...
This is the infix dereference operator (AKA The Arrow Operator).  It 
either fetches the value associated with Y (C<< X->[Y] >> and C<< X->{Y} >>)
from what X refers to or calls the function X refers to with an argument
list of Y (C<< X->(Y) >>).
...
This is the prefix auto-increment operator.  It is roughly equivalent to
C<X = X + 1>, but there is a bit of extra magic to it.  If
...
This is the postfix auto-increment operator.  It behaves the same way as 
the prefix auto-increment operator C<++X>, including the magic, but the value
is taken before the incrementing is done. So, the after the following code 
...
This is the prefix auto-decrement operator.  It is equivalent to C<X = X - 1>.
The value returned is reflects the decementing, so after the following code 
runs, C<$x> and C<$y> will be 4.
...
This is the postfix auto-decrement operator.  It is equivalent to C<X = X - 1>.
The value returned is the value before decementing, so after the following code 
runs, C<$x> will be 4 and C<$y> will be 5.
...
This is the exponentiation operator.  It raises X to the Yth power.  Warning:
it binds more tightly than unary minus, so -2**4 is -(2**4), not (-2)**4. 
...
This is the high-precedence logical negation operator.  It performs
logical negation, i.e., "not".  There is a low-prcedence version C<not>.
...
This is the bitwise negation operator (AKA 1's complement operator).  The
width of the result is platform-dependent: ~0 is 32 bits wide on a 32-bit
platform, but 64 bits wide on a 64-bit platform, so if you are expecting a
certain bit width, remember to use the & operator to mask off the excess 
bits.
...
This is the backslash operator (AKA the reference operator).  It creates 
a reference to X.  
...
strings.  It is useful syntactically for separating a function name from
a parenthesized expression that would otherwise be interpreted as the 
...
Y against a string X.  When used in scalar context, the return value 
generally indicates the success of the operation.  Behavior in list 
context depends on the particular operator.  See 
L</"Regexp Quote-Like Operators"> for details and L<perlretut> for 
examples using these operators.

If Y is an expression rather than a search pattern, substitution, 
or transliteration, it is interpreted as a search pattern at run
...
This is the negative binding operator.  It behaves like the =~ operator, but
the return is logically negated.
...
platform specific).  When Y is negative the reults are also not well 
...
one bit is the the same as an integer divison by 2.

When Y is negative the reults are not well defined (i.e. platform specific).
This is because the right bit-shift operator is implemented by the native C
compiler's right bit-shift operator  (and that is not defined by ISO C).
...
Y.  As always, strings in numeric context are converted to numbers before
comparison, and if they cannot be they are turned into C<0> (and a warning
is thrown if L<< C<warnings> >> are turned on).
...
X is greater than to Y.  As always, strings in numeric context are 
converted to numbers before comparison, and if they cannot be they are 
turned into C<0> (and a warning is thrown if L<< C<warnings> >> are 
turned on).
