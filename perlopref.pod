...
This is the high-precedence logical and operator.  It returns Y if X and
Y both evaluate to a true value or they both evaluate to a false value,
...
the X is true then Y is returned otherwise Z is returned.
...
That is it assigns Y to X if both X and Y are true or X is true and Y is
false.  If X is false it assigns X to itself.  This means that X must be a
