...
warning is thrown if L<warnings> are turned on).
...
This is L<perlop/The Arrow Operator>.
...
=head3 Example
    ...
    @{$aref}[0,1] = (1, 2)      #@a is now (1, 2, "c", "d")

    my %h      = (a => 1, b => 2, c => 3);
    my $href   = \%h;
    my $y      = $href->{b}; #$y is now 2
    $href->{d} = 4;          #%h is now (a => 1, b => 2, c => 3, d => 4)
    @{$href}{qw/a b c d/} = (0, 0, 0, 0);
    #%h is now (a => 0, b => 0, c => 0, d => 0)
...
unnecessary.  The use of this operator is discouraged, see the L<< /X->Y >>
operator for a better solution.
...
evaluates to a array reference then the value of the referenced item it
returned.  If X does not evaluate to a array reference a runtime error
occurs.  If X is a simple array variable the braces are unnecessary.
...
negation, i.e., "not".  There is a high-precedence version C<!>.
...
This is the low-precedence logical and operator.  It returns X if X
evaluates to false, otherwise it returns Y.
...
This is the low-precedence logical or operator.  It returns the X if X
evaluates to true, otherwise it returns Y.
